...
- Graffiti gives direct `.card > header` and `.card > footer` special chrome: edge-to-edge bleed, negative margins, borders, and clipped overflow behavior. Use direct `header`/`footer` only when that visual card chrome is desired; for dense list records, put title/actions in internal groups such as `<div class="cluster ...">` instead.
...
17. **Run visual verification for application UI edits**
    - For real app/page changes, especially card/list/dashboard layouts, verify in the browser with screenshot/vision after editing.
    - DOM/text snapshots are not enough: Graffiti component chrome, overflow clipping, title scale, density, and viewport scroll behavior can be broken while markup/data checks pass.

18. **Run aesthetic compliance checks**
...
19. **Emit output using required contract**
...
20. **Handle ambiguity with deterministic defaults**
